...
import {
  ACCESSORY_LIST,
  accessoryAssetPath,
  PENGUIN_ACCESSORY_IDS,
  PENGUIN_CLOTHES_FACINGS,
  penguinClothesOverlayKey,
  penguinClothesOverlayPath,
...
export class BootScene extends Phaser.Scene {
  ...
  preload() {
    ...
    for (const acc of ACCESSORY_LIST) {
      // Pet clothes load here; penguin body clothes use plate overlays below.
      const path = accessoryAssetPath(acc.id);
      if (path) this.load.image(acc.texture, path);
    }
    // Imagine-authored penguin clothes overlays (idle plate alignment).
    for (const id of PENGUIN_ACCESSORY_IDS) {
      for (const facing of PENGUIN_CLOTHES_FACINGS) {
        this.load.image(
          penguinClothesOverlayKey(id, facing),
          penguinClothesOverlayPath(id, facing),
        );
      }
    }
    // Classic Club Penguin plates for the player penguin.
    // Generated by: npm run sprite:penguin-classic
    // Frame 0 = idle plant; 1–8 = Tenor walk GIF cycle.
    for (const facing of ['down', 'up', 'side', 'se', 'sw', 'ne', 'nw'] as const) {
