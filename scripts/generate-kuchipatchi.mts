...
const REF = path.resolve('scripts/reference/kuchipatchi/frames');
const OUT = path.resolve('public/assets/pet/kuchipatchi');
const W = 32;
const H = 32;
...
function contentBounds(png: InstanceType<typeof PNG>) {
  ...
  if (!n) throw new Error('empty kuchipatchi frame');
  return { x0, y0, x1, y1 };
}
...
console.log('Kuchipatchi iD sprites written to', OUT);
