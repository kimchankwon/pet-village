...
const REF = path.resolve('scripts/reference/mametchi/frames');
const OUT = path.resolve('public/assets/pet/mametchi');
const W = 32;
const H = 32;
...
function contentBounds(png: InstanceType<typeof PNG>) {
  ...
  if (!n) throw new Error('empty mametchi frame');
  return { x0, y0, x1, y1 };
}
...
console.log('Mametchi iD sprites written to', OUT);
