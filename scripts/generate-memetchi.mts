...
const REF = path.resolve('scripts/reference/memetchi/frames');
const OUT = path.resolve('public/assets/pet/memetchi');
const W = 32;
const H = 32;
...
function contentBounds(png: InstanceType<typeof PNG>) {
  ...
  if (!n) throw new Error('empty memetchi frame');
  return { x0, y0, x1, y1 };
}
...
// mid-pose file cannot leave public/assets/pet/memetchi half-updated.
...
for (const { pose, canvas } of loaded) {
  // Gallery art already has a dark outline (~0,0,100). repairOutline would
...
console.log('Memetchi iD sprites written to', OUT);
