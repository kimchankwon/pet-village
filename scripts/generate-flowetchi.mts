...
const REF = path.resolve('scripts/reference/flowetchi/frames');
const OUT = path.resolve('public/assets/pet/flowetchi');
const W = 32;
const H = 32;
...
function contentBounds(png: InstanceType<typeof PNG>) {
  ...
  if (!n) throw new Error('empty flowetchi frame');
  return { x0, y0, x1, y1 };
}
...
console.log('Flowetchi iD sprites written to', OUT);
